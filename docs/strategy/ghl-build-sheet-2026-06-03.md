...
| `harvest-shop-interest` | wants to talk shop shelf | workflows 3 + 5 |
| `shop-call-booked` | booked a shop chat | workflow 3 |
| `shop-nurture-sent` | got the shop follow-up | workflow 5 |

Shop offer branch tags (already on contacts from the EOI): `shop-produce`, `shop-maker`,
`shop-food`, `shop-consignment`, `shop-follow-up`. Full tag library: `harvest-ghl-tag-and-automation-map.md`.
...
- **Action:** Add Tag `harvest-shop-interest`
...
- **Action:** Add Tag `witta-gathering-2026-06-20`
- **Settings:** Re-entry OFF
- **Then:** Publish → paste the link into `GardenLaunch.tsx:22` `IM_COMING_URL` → click it yourself → check both tags → `npm run count:rsvps:ghl` → remove the test tag
- [ ] link built   [ ] workflow built   [ ] pasted into page   [ ] tested
...
- **Settings:** Re-entry OFF
- **Then:** Publish → test with a dummy `harvest-shop-interest` contact
- [ ] built   [ ] tested
